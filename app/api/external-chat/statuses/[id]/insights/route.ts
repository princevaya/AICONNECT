...
import { getStatusInsights } from "@/services/external-chat/status.service";
...
export async function GET(
  ...
  try {
    ...
    const insights = await getStatusInsights(id, user);
    return NextResponse.json(insights);
  } catch (error) {
    console.error("[STATUS_INSIGHTS_GET]", error);
