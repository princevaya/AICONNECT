...
export default function ExternalChatApp() {
  ...
  const sendRequest = async (clerkId: string) => {
    ...
    try {
      await api("/api/external-chat/connections", {
        method: "POST",
      ...
      pushNotification({
        ...
        message: "The user will receive your request."
      });
    } catch (e) {
      handleApiError(e, "Failed to send request");
    }
  };
  ...
  const startDirectMessageWithUser = async (targetUser: UserRow) => {
    ...
    try {
      const roomName = targetUser.name || targetUser.email?.split("@")[0] || "Chat";
      ...
      const data = await api<{ room: Room }>("/api/external-chat/rooms", {
        ...
        body: JSON.stringify({
          name: roomName,
          type: "direct",
          memberClerkIds: [targetUser.clerkId],
        }),
      });
      ...
      pushNotification({
        level: "success",
        title: "Chat Started",
        message: `Conversation with ${roomName}`
      });
    } catch (err) {
      handleApiError(err, "Failed to start direct message");
    }
  };
  ...
  return (
    <div className="whatsapp-scrollbar">
      <ChatShell className="whatsapp-shell">
        <ChatPanel className="hidden p-3 xl:block whatsapp-panel bg-white/80 dark:bg-[#1F2C33]/80">
          <div className="mb-3 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <MessageSquare className="h-4 w-4 text-[#25D366]" />
              <p className="text-sm font-semibold text-[#111B21] dark:text-[#E9EDEF]">Conversations</p>
            </div>
            <ThemeToggle />
          </div>

          <div className="mb-3 rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-2.5">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <div className="h-9 w-9 overflow-hidden rounded-full border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#25D366]/15">
                  <UserAvatar
                    user={selfUser || { id: "self", clerkId: "self", name: "You", email: null, imageUrl: profileImageUrl }}
                    className="h-full w-full object-cover"
                    fallbackClassName="flex h-full w-full items-center justify-center text-xs font-semibold text-[#25D366]"
                  />
                </div>
                <div>
                  <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">My Profile</p>
                  <p className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">Account & preferences</p>
                </div>
              </div>
              <Button size="sm" variant="outline" onClick={() => setProfileSettingsOpen(true)} className="border-[#E9EDEF] dark:border-[#2A3942] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">
                <Settings2 className="h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" />
              </Button>
            </div>
          </div>

          <div className="mb-3 space-y-2 rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/35 dark:bg-[#2A3942]/35 p-2">
            <div className="flex gap-2">
              <div className="relative flex-1">
                <Search className="absolute left-2 top-2.5 h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" />
                <Input className="pl-8 bg-white dark:bg-[#1F2C33] border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search users..." />
              </div>
              <Button type="button" variant="outline" size="sm" onClick={() => setCreateGroupOpen(true)} className="border-[#25D366] text-[#25D366] hover:bg-[#25D366]/10">
                <PlusCircle className="mr-1 h-4 w-4" />
                Group
              </Button>
            </div>
            <div className="max-h-36 space-y-1 overflow-y-auto">
              {searching ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">Searching...</p> : null}
              {results.map((u) => (
                <div key={u.id} className="flex items-center justify-between rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-white/65 dark:bg-[#1F2C33]/65 px-2 py-1.5">
                  <div className="min-w-0">
                    <p className="truncate text-xs font-medium text-[#111B21] dark:text-[#E9EDEF]">{userLabel(u)}</p>
                    <p className="truncate text-[11px] text-[#667781] dark:text-[#AEBAC1]">{u.email || "No email provided"}</p>
                  </div>
                  <div className="flex gap-1">
                    <Button
                      ...
                      className={selectedGroupMemberIds.includes(u.clerkId) ? "bg-[#25D366] hover:bg-[#128C7E]" : "border-[#E9EDEF] dark:border-[#2A3942]"}
                    >
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void startDirectMessageWithUser(u)}
                      className="border-[#E9EDEF] dark:border-[#2A3942] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                      title="Start 1:1 Conversation"
                    >
                      <MessageSquare className="h-4 w-4 text-[#25D366]" />
                    </Button>
                  </div>
            ...
            {selectedGroupMemberIds.length > 0 ? (
              <p className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">{selectedGroupMemberIds.length} selected for group creation</p>
            ) : null}
          </div>

          <SidebarFilterTabs active={filterTab} onChange={setFilterTab} />

          <div className="mb-3 rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-2">
            <p className="mb-1 text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Incoming Requests</p>
            <div className="max-h-28 space-y-1 overflow-y-auto">
              {incoming.length === 0 ? <p className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">No pending requests</p> : null}
              {incoming.map((r) => (
                <div key={r.id} className="flex items-center justify-between rounded border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 px-2 py-1">
                  <p className="truncate text-xs text-[#111B21] dark:text-[#E9EDEF]">{userLabel(r.sender)}</p>
                  <div className="flex gap-1">
                    <Button size="sm" variant="ghost" onClick={() => void actRequest(r.id, "accept")}><Check className="h-4 w-4 text-[#25D366]" /></Button>
                    <Button size="sm" variant="ghost" onClick={() => void actRequest(r.id, "decline")}><X className="h-4 w-4 text-[#EF4444]" /></Button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mb-3 rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-2">
            <p className="mb-1 text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Starred Messages</p>
            <div className="max-h-28 space-y-1 overflow-y-auto">
              {bookmarkedMessages.length === 0 ? <p className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">No starred messages</p> : null}
              {bookmarkedMessages.slice(-3).reverse().map((m) => (
                <button
                  ...
                  className="w-full rounded border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 px-2 py-1 text-left hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]/60"
                >
                  <p className="line-clamp-2 text-xs text-[#111B21] dark:text-[#E9EDEF]">{m.content || "Attachment"}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="max-h-[calc(100%-330px)] space-y-1 overflow-y-auto pr-1">
            {loadingRooms ? <div className="flex items-center text-sm text-[#667781] dark:text-[#AEBAC1]"><Loader2 className="mr-2 h-4 w-4 animate-spin text-[#25D366]" />Loading chats...</div> : null}
            {filteredRooms.map((room) => {
              ...
              return (
                <div
                  ...
                  className={`chat-row-hover w-full rounded-lg border px-3 py-2 text-left transition-all ${activeRoomCode === room.code ? "chat-row-active" : "border-[#E9EDEF]/70 dark:border-[#2A3942]/70"
                    } cursor-pointer`}
                >
                  <div className="flex items-start gap-2">
                    <div className="relative">
                      <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-[#25D366]/15 text-xs font-semibold text-[#25D366]">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <p className="truncate text-sm font-medium text-[#111B21] dark:text-[#E9EDEF]">{room.name}</p>
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="rounded p-0.5 text-[#667781] dark:text-[#AEBAC1] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942] hover:text-[#111B21] dark:hover:text-[#E9EDEF]"
                          >
                            <Pin className={`h-3.5 w-3.5 ${starred ? "text-[#25D366]" : ""}`} />
                          </button>
                          {room.unreadCount > 0 ? <span className="rounded-full bg-[#25D366] px-2 py-0.5 text-[10px] text-white">{room.unreadCount}</span> : null}
                        </div>
                      </div>
            ...
            {archivedRooms.length > 0 ? (
              <div className="mt-3 rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/20 dark:bg-[#2A3942]/20 p-2">
                <button
                  type="button"
                  className="mb-2 flex w-full items-center justify-between text-left text-xs font-semibold uppercase tracking-wide text-[#667781] dark:text-[#AEBAC1]"
                ...
                {archivedOpen ? <div className="space-y-1">
                  {archivedRooms.map((room) => {
                    ...
                    return (
                      <div key={`archived-${room.id}`} className="rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-white/50 dark:bg-[#1F2C33]/50 p-2">
                        <button
                          type="button"
                          onClick={() => setActiveRoomCode(room.code)}
                        ...
                        >
                          <p className="truncate text-sm font-medium text-[#111B21] dark:text-[#E9EDEF]">{room.name}</p>
                          <p className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">Read-only archived chat</p>
                        </button>
                        <div className="mt-2 flex items-center gap-1">
                          {canUnarchive ? (
                            <Button size="sm" variant="outline" onClick={() => void toggleArchiveRoom(false, room.code)} className="border-[#25D366] text-[#25D366] hover:bg-[#25D366]/10">
                              Unarchive
                            </Button>
                          ) : null}
                          <Button size="sm" variant="ghost" onClick={() => void deleteChatForMe(room.code)} className="text-[#EF4444] hover:bg-[#EF4444]/10">
        ...
        <ChatPanel className={`flex min-h-0 flex-col ${infoPanelCollapsed ? "xl:col-span-2" : ""} whatsapp-panel bg-white/80 dark:bg-[#1F2C33]/80`}>
          <div className="sticky top-0 z-10 border-b border-[#E9EDEF]/60 dark:border-[#2A3942]/60 bg-white/75 dark:bg-[#1F2C33]/75 p-3 backdrop-blur chat-header">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Button
                  type="button"
                  size="icon"
                  variant="ghost"
                  className="xl:hidden text-[#667781] dark:text-[#AEBAC1] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                ...
                <div>
                  <p className="text-sm font-semibold text-[#111B21] dark:text-[#E9EDEF]">{activeRoom?.name || "Select a conversation"}</p>
                  <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">{activeRoomSubtitle}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button size="sm" variant="outline" onClick={() => router.push("/dashboard")} className="border-[#E9EDEF] dark:border-[#2A3942] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">
                  <LayoutDashboard className="mr-1 h-4 w-4 text-[#25D366]" /> Dashboard
                </Button>
                <Button
                  size="sm"
                  variant={infoPanelCollapsed ? "outline" : "default"}
                  className={`hidden xl:inline-flex ${infoPanelCollapsed ? "border-[#E9EDEF] dark:border-[#2A3942]" : "bg-[#25D366] hover:bg-[#128C7E] text-white"}`}
                  onClick={() => setInfoPanelCollapsed((v) => !v)}
                >
                  {infoPanelCollapsed ? "Show Info" : "Hide Info"}
                </Button>
                <div className="relative">
                  <Button size="sm" variant="outline" onClick={() => setNotificationsOpen((v) => !v)} className="border-[#E9EDEF] dark:border-[#2A3942] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">
                    <Bell className="h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" />
                  </Button>
                  {inAppNotifications.length > 0 ? (
                    <span className="absolute -right-1 -top-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-[#25D366] px-1 text-[10px] text-white">
                      {inAppNotifications.length > 9 ? "9+" : inAppNotifications.length}
                    </span>
                  ) : null}
                </div>
                <Button size="sm" variant="outline" onClick={() => setCallOverlayOpen(true)} className="border-[#25D366] text-[#25D366] hover:bg-[#25D366]/10">
                  <Phone className="h-4 w-4" />
                </Button>
                {activeRoom ? (
                  <div className="relative">
                    <Button size="sm" variant="outline" onClick={() => setMenuOpenForRoomCode((p) => p === activeRoom.code ? null : activeRoom.code)} className="border-[#E9EDEF] dark:border-[#2A3942] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">
                      <MoreVertical className="h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" />
                    </Button>
                    {menuOpenForRoomCode === activeRoom.code ? (
                      <div className="absolute right-0 z-20 mt-1 w-56 rounded-md border border-[#E9EDEF] dark:border-[#2A3942] bg-white dark:bg-[#1F2C33] p-1 shadow-lg">
                        <button
                          className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                          onClick={() => {
                            setCallOverlayOpen(true);
                            setMenuOpenForRoomCode(null);
                          }}
                        >
                          <Phone className="h-4 w-4 text-[#25D366]" /> Start Call
                        </button>
                        <button
                          className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                          onClick={() => {
                        ...
                        >
                          <CheckCheck className="h-4 w-4 text-[#25D366]" /> Mark Read
                        </button>
                        <button
                          className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                          onClick={() => {
                        ...
                        {activeRoom?.type === "group" ? (
                          ...
                          >
                            <UserPlus className="h-4 w-4 text-[#25D366]" /> Share Invite
                          </button>
                        ) : null}
                        {activeRoom?.type === "group" ? (
                          <button
                        ...
                        {activeRoom?.type === "direct" ? (
                          <button
                            className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#EF4444] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                            onClick={() => {
            ...
            {notificationsOpen ? (
              <div className="mt-2 rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-white/95 dark:bg-[#1F2C33]/95 p-2 shadow-sm">
                <div className="mb-2 flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wide text-[#667781] dark:text-[#AEBAC1]">Notifications</p>
                  {inAppNotifications.length > 0 ? (
                    <button
                      type="button"
                      className="text-[11px] text-[#667781] dark:text-[#AEBAC1] underline"
                ...
                <div className="max-h-40 space-y-1 overflow-y-auto">
                  {inAppNotifications.length === 0 ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">No notifications</p> : null}
                  {inAppNotifications.map((item) => (
                    <div
                      key={item.id}
                      className={`rounded border px-2 py-1.5 text-xs ${item.level === "error"
                        ? "border-[#EF4444]/30 bg-[#EF4444]/10"
                        : item.level === "success"
                          ? "border-[#25D366]/30 bg-[#25D366]/10"
                          : "border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25"
                        }`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-medium text-[#111B21] dark:text-[#E9EDEF]">{item.title}</p>
                          {item.message ? <p className="text-[#667781] dark:text-[#AEBAC1]">{item.message}</p> : null}
                        </div>
                        <button type="button" className="text-[#667781] dark:text-[#AEBAC1]" onClick={() => dismissNotification(item.id)}>
            ...
            <div className="mt-3 relative">
              <Search className="absolute left-2 top-2.5 h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" />
              <Input ref={searchInputRef} className="pl-8 bg-white dark:bg-[#1F2C33] border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search messages..." />
            </div>
            {search.trim() ? (
              <div className="mt-2 flex items-center justify-between gap-2 rounded-md border border-[#E9EDEF]/60 dark:border-[#2A3942]/60 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 px-2 py-1.5">
                ...
                <div className="flex items-center gap-1">
                  <Button type="button" size="icon" variant="ghost" onClick={() => jumpMatch(-1)} disabled={!messageMatches.length} className="hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">
              ...
              <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
                {pinnedMessages.map((m) => (
                  <button
                    ...
                    className="inline-flex max-w-[220px] shrink-0 items-center gap-1 rounded-full border border-[#25D366]/35 bg-[#25D366]/10 px-2 py-1 text-xs hover:bg-[#25D366]/20"
                  >
                    <Pin className="h-3 w-3 text-[#25D366]" />
                    <span className="truncate text-[#111B21] dark:text-[#E9EDEF]">{m.content || "Pinned attachment"}</span>
                  </button>
                ))}
              </div>
            ) : null}
          ...
          {!networkOnline ? <div className="border-t border-[#25D366]/30 bg-[#25D366]/10 px-3 py-2 text-xs text-[#25D366]">You are offline. Messages will sync once connection returns.</div> : null}
          {networkOnline && !realtimeConnected ? <div className="border-t border-[#25D366]/30 bg-[#25D366]/10 px-3 py-2 text-xs text-[#25D366]">Realtime disconnected. Reconnecting…</div> : null}
          {error ? (
            <div className="flex items-center justify-between gap-2 border-t border-[#EF4444]/20 bg-[#EF4444]/10 px-3 py-2 text-xs text-[#EF4444]">
              <span>{error}</span>
              <button type="button" onClick={() => setError("")} className="rounded px-1 hover:bg-[#EF4444]/15">Dismiss</button>
            </div>
          ) : null}

          <div
            ref={messageListRef}
            className={`min-h-0 flex-1 overflow-y-auto p-3 ${dropActive ? "bg-[#25D366]/5" : ""}`}
            onScroll={(e) => {
          ...
          >
            {dropActive ? (
              <div className="mb-2 rounded-lg border border-dashed border-[#25D366]/50 bg-[#25D366]/10 px-3 py-2 text-xs text-[#25D366]">
                Drop file to upload
              </div>
            ) : null}
            {messagesHasMore ? (
              <div className="mb-3 flex justify-center">
                <Button
                  ...
                  className="border-[#E9EDEF] dark:border-[#2A3942] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                >
                  {loadingOlderMessages ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin text-[#25D366]" /> : null}
                  {loadingOlderMessages ? "Loading older..." : "Load older messages"}
                </Button>
              </div>
            ) : null}
            {loadingMessages ? <div className="flex items-center text-sm text-[#667781] dark:text-[#AEBAC1]"><Loader2 className="mr-2 h-4 w-4 animate-spin text-[#25D366]" />Loading messages...</div> : null}
            {!loadingMessages && messages.length === 0 ? <p className="text-sm text-[#667781] dark:text-[#AEBAC1]">No messages yet.</p> : null}
            <div className="space-y-3">
              {visibleTimelineItems.map((item) => {
                if (item.kind === "day") {
                  return (
                    <div key={item.key} className="my-2 flex items-center justify-center">
                      <span className="rounded-full border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/30 dark:bg-[#2A3942]/30 px-3 py-1 text-[11px] text-[#667781] dark:text-[#AEBAC1]">{item.label}</span>
                    </div>
                  );
                }
                if (item.kind === "unread") {
                  ...
                  <div key={m.id} className={`flex ${own ? "justify-end" : "justify-start"}`}>
                    <article
                      ...
                      className={`group inline-flex h-auto w-auto max-w-[82%] flex-col items-start rounded-2xl border ${compactMode ? "p-2.5" : "p-3"} transition ${m.type === "note"
                        ...
                        } ${messageMatches[messageSearchIndex] === m.id ? "ring-2 ring-[#25D366]/70" : ""}`}
                    ...
                    >
                      <div className="mb-1 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                          <div className="h-6 w-6 overflow-hidden rounded-full border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#25D366]/15">
                            <UserAvatar
                              user={m.sender}
                              className="h-full w-full object-cover"
                          ...
                          <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">{own ? "You" : userLabel(m.sender)}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          {m.optimistic ? <span className="text-[10px] text-[#25D366]">Sending...</span> : null}
                          {m.failed ? <span className="text-[10px] text-[#EF4444]">Failed</span> : null}
                          <p className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">{new Date(m.createdAt).toLocaleString()}</p>
                        </div>
                      </div>
                      {m.replyToId ? (
                        <div className="mb-2 max-w-full rounded-md border border-[#E9EDEF]/60 dark:border-[#2A3942]/60 bg-[#F0F2F5]/35 dark:bg-[#2A3942]/35 px-2 py-1.5">
                          <div className="mb-0.5 border-l-2 border-[#25D366] pl-2">
                            ...
                            <p className="line-clamp-1 text-[11px] text-[#667781] dark:text-[#AEBAC1]">
                              {messageById.get(m.replyToId)?.content || replyText || "Message"}
                            </p>
                          </div>
                        </div>
                      ) : null}
                      <p className={`${messageTextClass} whitespace-pre-wrap break-words text-[#111B21] dark:text-[#E9EDEF]`}>{renderMessageContent(m.content, search)}</p>
                      {m.attachment ? (
                        isImageAttachment(m.attachment) ? (
                          <button
                            type="button"
                            onClick={() => setLightboxImage({ src: m.attachment?.downloadUrl || "", alt: m.attachment?.fileName || "image" })}
                            className="mt-2 block overflow-hidden rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70"
                          >
                          ...
                          <div className="mt-2 rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-2">
                            <p className="mb-1 text-xs font-medium text-[#111B21] dark:text-[#E9EDEF]">{m.attachment.fileName}</p>
                            <audio controls className="w-full">
                              <source src={m.attachment.downloadUrl} />
                            </audio>
                          </div>
                        ) : (
                          <a href={m.attachment.downloadUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs underline text-[#25D366]"><FileImage className="h-3.5 w-3.5" /> {m.attachment.fileName}</a>
                        )
                      ) : null}
                      {(() => {
                        ...
                        return (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="mt-2 block rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/20 dark:bg-[#2A3942]/20 p-2 hover:bg-[#F0F2F5]/50 dark:hover:bg-[#2A3942]/50"
                          >
                            <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">{preview.title}</p>
                            {preview.description ? <p className="mt-1 line-clamp-2 text-xs text-[#667781] dark:text-[#AEBAC1]">{preview.description}</p> : null}
                            <p className="mt-1 text-[11px] text-[#667781] dark:text-[#AEBAC1]">{preview.siteName || new URL(preview.url).hostname}</p>
                          </a>
                        );
                      })()}
                      {reactionsAgg.length > 0 ? (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {reactionsAgg.map((r) => (
                            <span key={`${m.id}-${r.emoji}`} className="rounded-full border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/30 dark:bg-[#2A3942]/30 px-2 py-0.5 text-xs">
                              {r.emoji} {r.count}
                            </span>
                          ))}
                        </div>
                      ...
                      {poll ? (
                        <div className="mt-2 rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/40 dark:bg-[#2A3942]/40 p-2">
                          <p className="mb-1 text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">{poll.question}</p>
                          <p className="mb-2 text-[11px] text-[#667781] dark:text-[#AEBAC1]">
                            ...
                            {(poll.options || []).map((o, idx) => {
                              ...
                              return (
                                <button
                                  ...
                                  className="mx-auto grid min-h-9 w-[96%] grid-cols-[minmax(0,1fr)_2.25rem] items-start gap-2 rounded border border-[#E9EDEF] dark:border-[#2A3942] px-2.5 py-1.5 text-xs hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                                  style={{
                                    backgroundImage: `linear-gradient(90deg, ${fill} ${percent}%, transparent ${percent}%)`,
                                  }}
                                >
                                  <span className="min-w-0 whitespace-normal break-words text-left leading-5 text-[#111B21] dark:text-[#E9EDEF]">{o.text}</span>
                                  <span className="pt-0.5 text-right tabular-nums text-[#667781] dark:text-[#AEBAC1]">{optionVotes}</span>
                                </button>
                              );
                            })}
                          </div>
                      ...
                      <div className="mt-2 flex items-center justify-between gap-2 opacity-100 transition md:opacity-0 md:group-hover:opacity-100">
                        <div className="flex items-center gap-1">
                          {m.editedAt ? <span className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">(edited)</span> : null}
                          {messageStatus(m)}
                        </div>
                        <div className="flex items-center gap-1">
                          <div className="relative">
                            <Button
                              ...
                              className="hover:bg-[#F0F2F5]/50 dark:hover:bg-[#2A3942]/50"
                            >
                              <MoreVertical className="h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" />
                            </Button>
                            {messageMenuOpenId === m.id ? (
                              <div
                                className={`absolute z-30 mt-1 w-48 max-w-[min(18rem,calc(100vw-2rem))] rounded-md border border-[#E9EDEF] dark:border-[#2A3942] bg-white dark:bg-[#1F2C33] p-1 shadow-lg ${own ? "right-0 bottom-full mb-1 origin-bottom-right" : "left-0 bottom-full mb-1 origin-bottom-left"
                                  }`}
                              >
                                <div className="mb-1 flex flex-wrap gap-1 px-1 py-1">
                                  {REACTIONS.map((emoji) => (
                                    <button
                                      ...
                                      className="rounded-full border border-[#E9EDEF] dark:border-[#2A3942] px-2 py-0.5 text-xs hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                                    >
                                      {emoji}
                                    </button>
                                  ))}
                                </div>
                                <button className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]" onClick={() => { setReplyToId(m.id); setMessageMenuOpenId(null); }}><Reply className="h-4 w-4" /> Reply</button>
                                <button className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]" onClick={() => { openThread(m); setMessageMenuOpenId(null); }}><ChevronRight className="h-4 w-4" /> Thread</button>
                                <button className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]" onClick={() => { toggleBookmark(m.id); setMessageMenuOpenId(null); }}>{bookmarkedMessageIds.includes(m.id) ? <StarOff className="h-4 w-4 text-[#25D366]" /> : <Star className="h-4 w-4" />} {bookmarkedMessageIds.includes(m.id) ? "Unstar" : "Star"}</button>
                                <button className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]" onClick={() => { void copyMessageContent(m); setMessageMenuOpenId(null); }}>Copy Message</button>
                                <button className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]" onClick={() => { void mutateMessage(m.id, { pinned: !Boolean(m.pinnedAt) }); setMessageMenuOpenId(null); }}>{m.pinnedAt ? "Unpin" : "Pin"}</button>
                                <button className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]" onClick={() => { const next = window.prompt("Edit message", m.content); if (next !== null) void mutateMessage(m.id, { content: next }); setMessageMenuOpenId(null); }}>Edit</button>
                                <button className="flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-[#EF4444] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]" onClick={() => { void deleteMessage(m.id); setMessageMenuOpenId(null); }}>Delete</button>
                              </div>
                            ) : null}
                          </div>
                          {m.failed ? (
                            <Button size="sm" variant="outline" onClick={() => void retryOptimisticMessage(m.id)} className="border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">Retry</Button>
                          ) : null}
                          <span className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">Seen by {m.seenBy.length}</span>
                          {m.seenBy.length > 0 ? (
                            <div className="ml-1 flex items-center -space-x-1">
                              {m.seenBy.slice(0, 3).map((s) => (
                                <span
                                  key={`${m.id}-seen-${s.userId}`}
                                  className="inline-flex h-5 w-5 items-center justify-center overflow-hidden rounded-full border border-background bg-[#25D366]/20 text-[10px] font-semibold text-[#111B21] dark:text-[#E9EDEF]"
            ...
            {pendingNewCount > 0 ? (
              <div className="pointer-events-none sticky bottom-3 mt-2 flex justify-center">
                <Button type="button" size="sm" className="pointer-events-auto rounded-full shadow-lg bg-[#25D366] hover:bg-[#128C7E] text-white" onClick={scrollToLatest}>
                  {pendingNewCount} new message{pendingNewCount > 1 ? "s" : ""} · jump
                </Button>
              </div>
            ) : null}
            {text.trim() ? (
              <div className="mt-2 flex items-center gap-2 text-xs text-[#667781] dark:text-[#AEBAC1]">
          ...
          {!canSendInActiveRoom && activeRoom ? (
            <div className="border-t border-[#25D366]/30 bg-[#25D366]/10 px-3 py-2 text-xs text-[#25D366]">
          ...
          <form ref={composerFormRef} onSubmit={sendMessage} className="chat-composer border-t border-[#E9EDEF]/60 dark:border-[#2A3942]/60 bg-white/75 dark:bg-[#1F2C33]/75 p-3 backdrop-blur">
            {undoSend ? (
              <div className="mb-2 flex items-center justify-between rounded-md border border-[#25D366]/30 bg-[#25D366]/10 px-2 py-1.5 text-xs">
                <span className="text-[#111B21] dark:text-[#E9EDEF]">Message sent. Undo?</span>
                <Button type="button" size="sm" variant="outline" onClick={undoLastSend} className="border-[#25D366] text-[#25D366] hover:bg-[#25D366]/10">Undo</Button>
              </div>
            ) : null}
            {replying ? (
              <div className="mb-2 flex items-start justify-between gap-2 rounded-md border border-[#25D366]/25 bg-[#25D366]/8 px-2 py-1.5">
                <div className="flex min-w-0 flex-1 items-start gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-[#667781] dark:text-[#AEBAC1]">{replying.content.slice(0, 90)}</p>
                  </div>
                </div>
                <button type="button" onClick={() => setReplyToId(null)} className="text-xs underline text-[#25D366]">Cancel</button>
              </div>
            ) : null}
            <div className="mb-2 grid gap-2 md:grid-cols-2">
              <select value={messageType} onChange={(e) => setMessageType(e.target.value as "text" | "note" | "poll")} className="h-9 rounded-md border border-[#E9EDEF] dark:border-[#2A3942] bg-white dark:bg-[#1F2C33] px-3 text-sm text-[#111B21] dark:text-[#E9EDEF]">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button type="button" variant="outline" className="w-full border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]" onClick={() => fileInputRef.current?.click()} disabled={uploading || !canSendInActiveRoom}>
                  {uploading ? <Loader2 className="h-4 w-4 animate-spin text-[#25D366]" /> : <Paperclip className="h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" />}<span className="ml-1">Attach</span>
                </Button>
              </div>
            </div>
            {attachment ? <div className="mb-2 rounded-md border border-[#E9EDEF] dark:border-[#2A3942] px-2 py-1 text-xs text-[#111B21] dark:text-[#E9EDEF]">Attached: {attachment.fileName}<button type="button" onClick={() => setAttachment(null)} className="ml-2 underline text-[#25D366]">Remove</button></div> : null}
            {messageType === "poll" ? (
              <div className="mb-2 space-y-2 rounded-md border border-[#E9EDEF] dark:border-[#2A3942] bg-[#F0F2F5]/30 dark:bg-[#2A3942]/30 p-2">
                <div className="flex items-center gap-2 text-xs font-medium text-[#111B21] dark:text-[#E9EDEF]"><Vote className="h-4 w-4 text-[#25D366]" />Poll builder</div>
                <Input value={pollQuestion} onChange={(e) => setPollQuestion(e.target.value)} placeholder="Poll question" className="bg-white dark:bg-[#1F2C33] border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]" />
                {pollOptions.map((opt, i) => (
                  <Input key={`opt-${i}`} value={opt} onChange={(e) => setPollOptions((p) => p.map((x, idx) => (idx === i ? e.target.value : x)))} placeholder={`Option ${i + 1}`} className="bg-white dark:bg-[#1F2C33] border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]" />
                ))}
                <Button type="button" size="sm" variant="outline" onClick={() => setPollOptions((p) => [...p, ""])} disabled={!canSendInActiveRoom} className="border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">Add option</Button>
              </div>
            ) : null}
            {messageType === "note" ? (
              <div className="mb-2 rounded-md border border-[#E9EDEF] dark:border-[#2A3942] bg-[#F0F2F5]/30 dark:bg-[#2A3942]/30 p-2">
                <p className="mb-2 text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Note Bubble Color</p>
                <div className="flex flex-wrap gap-2">
                  {NOTE_COLORS.map((entry) => (
                    <button
                      ...
                      className={`rounded-full border px-2.5 py-1 text-xs ${entry.className} ${noteColor === entry.id ? "ring-2 ring-[#25D366]/60" : ""}`}
              ...
              <div className="flex items-center gap-1">
                {["👍", "❤️", "😂", "🔥", "🙏"].map((emoji) => (
                  <button
                    ...
                    disabled={!canSendInActiveRoom}
                    className="rounded-full border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 px-2 py-0.5 text-xs hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]"
                  >
                    {emoji}
                  </button>
                ))}
              </div>
              <span className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">{activeRoomCode && draftByRoom[activeRoomCode] ? "Draft saved" : "Use /poll /note /text /call"}</span>
            </div>
            <div className="flex items-end gap-2">
              ...
              <div className="relative flex-1">
                <Textarea
                  ...
                  className="min-h-[54px] max-h-40 bg-white dark:bg-[#1F2C33] border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]"
                  placeholder="Type message. Use @username for mentions."
                  disabled={!canSendInActiveRoom}
                />
                {mentionOpen ? (
                  <div className="absolute bottom-[calc(100%+6px)] left-0 right-0 z-20 max-h-44 overflow-y-auto rounded-md border border-[#E9EDEF] dark:border-[#2A3942] bg-white dark:bg-[#1F2C33] p-1 shadow-lg">
                    {mentionLoading ? <p className="px-2 py-1 text-xs text-[#667781] dark:text-[#AEBAC1]">Searching...</p> : null}
                    {!mentionLoading && mentionResults.length === 0 ? <p className="px-2 py-1 text-xs text-[#667781] dark:text-[#AEBAC1]">No matches</p> : null}
                    {mentionResults.map((u, i) => (
                      <button
                        ...
                        className={`flex w-full items-center justify-between rounded px-2 py-1 text-left ${mentionIndex === i ? "bg-[#F0F2F5] dark:bg-[#2A3942]" : "hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"}`}
                      >
                        <span className="text-xs font-medium text-[#111B21] dark:text-[#E9EDEF]">{userLabel(u)}</span>
                        <span className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">@{mentionToken(u)}</span>
                      </button>
                    ))}
                  </div>
                ) : null}
              </div>
              <Button type="submit" disabled={sending || uploading || !activeRoomCode || !canSendInActiveRoom} className="transition-transform duration-150 active:scale-95 bg-[#25D366] hover:bg-[#128C7E] text-white">
                {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
              </Button>
            </div>
            {recording ? (
              <div className="mt-2 flex items-center justify-between rounded-md border border-[#EF4444]/35 bg-[#EF4444]/10 px-2 py-1.5">
                <div className="flex items-center gap-1.5">
                  {[0, 1, 2, 3, 4].map((i) => (
                    <span
                      key={`wave-${i}`}
                      className="block w-1 rounded bg-[#EF4444] animate-pulse"
                      style={{ height: `${8 + (i % 3) * 4}px`, animationDelay: `${i * 0.12}s` }}
                    />
                  ))}
                  <span className="text-xs text-[#EF4444]">Recording {formatDuration(recordingMs)}</span>
                </div>
                <Button type="button" size="sm" variant="outline" onClick={() => mediaRecorderRef.current?.stop()} className="border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">
                  Stop
                </Button>
              </div>
            ) : null}
        ...
        <ChatPanel className={`${infoPanelCollapsed ? "hidden" : "hidden xl:flex"} min-h-0 flex-col p-3 whatsapp-panel bg-white/80 dark:bg-[#1F2C33]/80`}>
          <div className="mb-3 border-b border-[#E9EDEF]/60 dark:border-[#2A3942]/60 pb-3">
            <p className="text-sm font-semibold text-[#111B21] dark:text-[#E9EDEF]">Info Panel</p>
            <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">Members, media and pinned context</p>
          </div>

          <div className="mb-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#667781] dark:text-[#AEBAC1]">Members</p>
            <div className="space-y-2">
              {!activeRoom?.members?.length ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">No room members loaded yet</p> : null}
              {activeRoom?.members?.map((member) => {
                ...
                return (
                  <div key={`${activeRoom?.id}-${member.userId}`} className="flex items-center justify-between rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 px-2 py-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-[#111B21] dark:text-[#E9EDEF]">{label}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      ...
                      {canManageThisMember ? (
                        <Button
                          ...
                          title="Remove member"
                          className="hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                        >
                      ...
                      {canTransferToMember ? (
                        <Button
                          ...
                          className="hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"
                        >
                          <Star className="h-3.5 w-3.5 text-[#25D366]" />
                        </Button>
                      ) : null}
                    </div>
                  </div>
          ...
          {activeRoom?.type === "direct" ? (
            <div className="mb-4">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#667781] dark:text-[#AEBAC1]">Shared Groups</p>
              <div className="space-y-2">
                {mutualGroups.length === 0 ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">No shared groups yet</p> : null}
                {mutualGroups.map((room) => (
                  <button
                    key={`mutual-${room.id}`}
                    type="button"
                    onClick={() => setActiveRoomCode(room.code)}
                    className="w-full rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/20 dark:bg-[#2A3942]/20 px-2 py-1.5 text-left hover:bg-[#F0F2F5]/60 dark:hover:bg-[#2A3942]/60"
                  >
                    <p className="truncate text-xs font-medium text-[#111B21] dark:text-[#E9EDEF]">{room.name}</p>
                    <p className="text-[10px] text-[#667781] dark:text-[#AEBAC1]">{room.members?.length || 0} members</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="mb-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#667781] dark:text-[#AEBAC1]">Mentions</p>
            <div className="space-y-2">
              {mentionNotifications.length === 0 ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">No recent mentions</p> : null}
              {mentionNotifications.map((m) => (
                <button
                  ...
                  className="w-full rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/20 dark:bg-[#2A3942]/20 px-2 py-1.5 text-left hover:bg-[#F0F2F5]/60 dark:hover:bg-[#2A3942]/60"
                >
                  <p className="text-[11px] font-medium text-[#111B21] dark:text-[#E9EDEF]">{userLabel(m.sender)}</p>
                  <p className="line-clamp-2 text-xs text-[#667781] dark:text-[#AEBAC1]">{m.content}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mb-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#667781] dark:text-[#AEBAC1]">Starred Messages</p>
            <div className="space-y-2">
              {bookmarkedMessages.length === 0 ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">No starred messages yet</p> : null}
              {bookmarkedMessages.slice(-6).map((m) => (
                ...
                >
                  <p className="line-clamp-2 text-xs text-[#111B21] dark:text-[#E9EDEF]">{m.content || "Attachment"}</p>
                  <p className="mt-1 text-[10px] text-[#667781] dark:text-[#AEBAC1]">{new Date(m.createdAt).toLocaleString()}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mb-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#667781] dark:text-[#AEBAC1]">Shared Media</p>
            <div className="grid grid-cols-3 gap-2">
              {sharedMedia.length === 0 ? <p className="col-span-3 text-xs text-[#667781] dark:text-[#AEBAC1]">No media shared yet</p> : null}
              {sharedMedia.map((m) => (
                <a
                  key={m.id}
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="aspect-square rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/30 dark:bg-[#2A3942]/30 p-2 hover:bg-[#F0F2F5]/50 dark:hover:bg-[#2A3942]/50"
                >
                  <FileImage className="h-4 w-4 text-[#25D366]" />
                  <p className="mt-1 line-clamp-2 text-[10px] text-[#667781] dark:text-[#AEBAC1]">{m.attachment?.fileName}</p>
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#667781] dark:text-[#AEBAC1]">Pinned Messages</p>
            <div className="space-y-2">
              {pinnedMessages.length === 0 ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">No pinned messages.</p> : null}
              {pinnedMessages.map((m) => (
                <button key={m.id} onClick={() => openThread(m)} className="w-full rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 px-2 py-1.5 text-left hover:bg-[#F0F2F5]/60 dark:hover:bg-[#2A3942]/60">
                  <p className="line-clamp-2 text-xs text-[#111B21] dark:text-[#E9EDEF]">{m.content}</p>
                  <p className="mt-1 text-[10px] text-[#667781] dark:text-[#AEBAC1]">{new Date(m.createdAt).toLocaleString()}</p>
                </button>
              ))}
            </div>
        ...
        {/* All modal dialogs remain exactly the same but with updated color classes */}
        ...
        <div className={`fixed inset-0 z-[55] transition ${profileSettingsOpen ? "pointer-events-auto" : "pointer-events-none"}`}>
          ...
          />
          <aside className={`absolute right-0 top-0 h-full w-full max-w-sm border-l border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-white/95 dark:bg-[#1F2C33]/95 p-4 backdrop-blur-xl transition-transform duration-300 ${profileSettingsOpen ? "translate-x-0" : "translate-x-full"}`}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-[#111B21] dark:text-[#E9EDEF]">Profile & Settings</p>
                <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">Personalize your chat experience</p>
              </div>
              <Button size="sm" variant="ghost" onClick={() => setProfileSettingsOpen(false)} className="hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"><X className="h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" /></Button>
            </div>

            <div className="space-y-3">
              <div className="rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-3">
                <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Profile Photo</p>
                <div className="mt-2 flex items-center gap-3">
                  <div className="h-16 w-16 overflow-hidden rounded-full border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#25D366]/15">
                    <UserAvatar
                      user={selfUser || { id: "self", clerkId: "self", name: "You", email: null, imageUrl: profileImageUrl }}
                      className="h-full w-full object-cover"
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button size="sm" variant="outline" onClick={() => profileImageInputRef.current?.click()} disabled={savingProfile} className="border-[#25D366] text-[#25D366] hover:bg-[#25D366]/10">
              ...
              <div className="rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-3">
                <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Notifications</p>
                <button className="mt-2 flex w-full items-center justify-between rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 px-2 py-1.5 text-sm hover:bg-[#F0F2F5]/60 dark:hover:bg-[#2A3942]/60" onClick={() => setNotificationSoundOn((v) => !v)}>
                  <span className="inline-flex items-center gap-2 text-[#111B21] dark:text-[#E9EDEF]"><Bell className="h-4 w-4 text-[#25D366]" /> Sound Alerts</span>
                  <span className="text-xs text-[#667781] dark:text-[#AEBAC1]">{notificationSoundOn ? "On" : "Off"}</span>
                </button>
              </div>
              <div className="rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-3">
                <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Privacy</p>
                <button className="mt-2 flex w-full items-center justify-between rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 px-2 py-1.5 text-sm hover:bg-[#F0F2F5]/60 dark:hover:bg-[#2A3942]/60" onClick={() => setPrivacyModeOn((v) => !v)}>
                  <span className="inline-flex items-center gap-2 text-[#111B21] dark:text-[#E9EDEF]"><Shield className="h-4 w-4 text-[#25D366]" /> Privacy Mode</span>
                  <span className="text-xs text-[#667781] dark:text-[#AEBAC1]">{privacyModeOn ? "Enabled" : "Disabled"}</span>
                </button>
              </div>
            </div>
          </aside>
        ...
        <div className={`fixed inset-0 z-[56] transition ${groupSettingsOpen ? "pointer-events-auto" : "pointer-events-none"}`}>
          ...
          />
          <aside className={`absolute right-0 top-0 h-full w-full max-w-md border-l border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-white/95 dark:bg-[#1F2C33]/95 p-4 backdrop-blur-xl transition-transform duration-300 ${groupSettingsOpen ? "translate-x-0" : "translate-x-full"}`}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-[#111B21] dark:text-[#E9EDEF]">Group Settings</p>
                <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">Manage this group without changing your personal profile.</p>
              </div>
              <Button size="sm" variant="ghost" onClick={() => setGroupSettingsOpen(false)} className="hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"><X className="h-4 w-4 text-[#667781] dark:text-[#AEBAC1]" /></Button>
            </div>
            {!activeRoom || activeRoom.type !== "group" ? (
              <p className="text-sm text-[#667781] dark:text-[#AEBAC1]">Open a group chat to edit its settings.</p>
            ) : (
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-1 rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-1">
                  {[
                    ...
                    <button
                      ...
                      className={`rounded-md px-2 py-1.5 text-xs font-medium ${groupSettingsTab === tab.id ? "bg-[#25D366] text-white" : "text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]"}`}
                    >
                      {tab.label}
                    </button>
                  ))}
                ...
                {groupSettingsTab === "general" ? (
                  <>
                    <div className="rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-3">
                      <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Group Profile</p>
                      <div className="mt-2 flex items-center gap-3">
                        <div className="h-14 w-14 overflow-hidden rounded-full border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#25D366]/15">
                          {groupAvatarDraft ? (
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <Button size="sm" variant="outline" onClick={() => groupAvatarInputRef.current?.click()} className="border-[#25D366] text-[#25D366] hover:bg-[#25D366]/10">
                            <Upload className="mr-1 h-4 w-4" /> Change
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => setGroupAvatarDraft("")} className="text-[#EF4444] hover:bg-[#EF4444]/10">Remove</Button>
                        </div>
                      </div>
                      <div className="mt-3 space-y-2">
                        <Input value={groupNameDraft} onChange={(e) => setGroupNameDraft(e.target.value)} placeholder="Group name" className="bg-white dark:bg-[#1F2C33] border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]" />
                        <Textarea value={groupDescriptionDraft} onChange={(e) => setGroupDescriptionDraft(e.target.value)} className="min-h-20 bg-white dark:bg-[#1F2C33] border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]" placeholder="Group description" />
                        <Button onClick={() => void saveGroupSettings()} disabled={!canManageMembers} className="bg-[#25D366] hover:bg-[#128C7E] text-white">Save Group Profile</Button>
                      </div>
                    </div>

                    <div className="rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-3">
                      <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Group Actions</p>
                      <div className="mt-2 grid grid-cols-2 gap-2">
                        {canManageMembers ? (
                          <Button size="sm" variant="outline" onClick={() => void toggleArchiveRoom(!Boolean(activeRoom.archivedAt))} className="border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">
                            {activeRoom.archivedAt ? "Unarchive" : "Archive"}
                          </Button>
                        ) : null}
                        {activeViewerRole === "owner" || activeViewerRole === "admin" ? (
                          <Button size="sm" variant="outline" onClick={() => void deassignSelfLeadership()} className="border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">Deassign My Role</Button>
                        ) : null}
                        <Button size="sm" variant="outline" onClick={() => void leaveActiveGroup()} className="border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF] hover:bg-[#F0F2F5] dark:hover:bg-[#2A3942]">Leave Group</Button>
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() => {
                ...
                {groupSettingsTab === "members" ? (
                  <div className="rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-3">
                    <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Add Members</p>
                    <Input
                      className="mt-2 bg-white dark:bg-[#1F2C33] border-[#E9EDEF] dark:border-[#2A3942] text-[#111B21] dark:text-[#E9EDEF]"
                      value={groupMemberSearch}
                      onChange={(e) => setGroupMemberSearch(e.target.value)}
                      placeholder="Search users to add"
                      disabled={!canManageMembers}
                    />
                    <div className="mt-2 max-h-48 space-y-1 overflow-y-auto rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 p-2">
                      {!canManageMembers ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">Only owner/admin can add members.</p> : null}
                      {canManageMembers && groupMemberSearchLoading ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">Searching...</p> : null}
                      {canManageMembers && !groupMemberSearchLoading && groupMemberSearch.trim() && groupMemberSearchResults.length === 0 ? (
                        <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">No eligible users found.</p>
                      ) : null}
                      {groupMemberSearchResults.map((u) => (
                        <button
                          ...
                          onClick={() => toggleGroupMember(u.clerkId)}
                          disabled={!canManageMembers}
                          className={`flex w-full items-center justify-between rounded border px-2 py-1.5 text-left text-sm ${selectedGroupMemberIds.includes(u.clerkId) ? "border-[#25D366] bg-[#25D366]/10" : "border-[#E9EDEF]/70 dark:border-[#2A3942]/70 hover:bg-[#F0F2F5]/60 dark:hover:bg-[#2A3942]/60"
                            }`}
                        >
                          <span className="truncate text-[#111B21] dark:text-[#E9EDEF]">{userLabel(u)}</span>
                          <span className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">{selectedGroupMemberIds.includes(u.clerkId) ? "Selected" : "Add"}</span>
                        </button>
                      ))}
                    </div>
                    <div className="mt-2 flex items-center justify-between">
                      <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">{selectedGroupMemberIds.length} selected</p>
                      <Button size="sm" variant="outline" disabled={!canManageMembers || selectedGroupMemberIds.length === 0} onClick={() => void addSelectedMembersToActiveGroup()} className="border-[#25D366] text-[#25D366] hover:bg-[#25D366]/10">
                        Add Selected
                      </Button>
                    </div>
                  </div>
                ...
                {groupSettingsTab === "activity" ? (
                  <div className="rounded-lg border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-[#F0F2F5]/25 dark:bg-[#2A3942]/25 p-3">
                    <p className="text-xs font-semibold text-[#111B21] dark:text-[#E9EDEF]">Group Activity</p>
                    <div className="mt-2 max-h-72 space-y-1.5 overflow-y-auto rounded-md border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 p-2">
                      {loadingGroupActivity ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">Loading activity...</p> : null}
                      {!loadingGroupActivity && groupActivityLogs.length === 0 ? <p className="text-xs text-[#667781] dark:text-[#AEBAC1]">No activity yet.</p> : null}
                      {groupActivityLogs.map((log) => (
                        <div key={`group-activity-${log.id}`} className="rounded border border-[#E9EDEF]/70 dark:border-[#2A3942]/70 bg-white/70 dark:bg-[#1F2C33]/70 px-2 py-1.5">
                          <p className="text-xs font-medium text-[#111B21] dark:text-[#E9EDEF]">{log.action.replaceAll("_", " ")}</p>
                          <p className="text-[11px] text-[#667781] dark:text-[#AEBAC1]">{userLabel(log.user)} • {new Date(log.createdAt).toLocaleString()}</p>
                        </div>
                      ))}
                    </div>
                  </div>
