...
interface MeetingRoomProps {
  ...
  onLeave: () => void;
  onPanelStateChange?: (chatOpen: boolean, participantsOpen: boolean, servicesHubOpen: boolean) => void;
}
...
const Toast = ({ message, type, onClose }: { message: string; type: 'success' | 'error' | 'info'; onClose: () => void }) => {
  ...
  return (
    <div className={`fixed bottom-24 left-1/2 transform -translate-x-1/2 z-${Z_INDEX.NOTIFICATION}
      px-4 py-2 rounded-lg shadow-lg animate-in slide-in-from-bottom-2 duration-300
      ${type === 'error' ? 'bg-red-500' : type === 'success' ? 'bg-green-500' : 'bg-blue-500'}
      text-white text-sm whitespace-nowrap md:whitespace-normal max-w-[90vw] md:max-w-md`}>
      {message}
    </div>
  );
};
  ...
  return (
    <>
      <button
        ...
        className="fixed top-16 right-3 z-[100] bg-black/70 text-white p-2 rounded-full hover:bg-black/90 transition-all md:top-20 md:right-4 shadow-lg backdrop-blur-sm"
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShow(false)}>
          <div className="bg-card rounded-lg p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span>🎤 Toggle Mic</span><kbd className="px-2 py-1 bg-muted rounded">M</kbd></div>
              <div className="flex justify-between"><span>📹 Toggle Camera</span><kbd className="px-2 py-1 bg-muted rounded">V</kbd></div>
              <div className="flex justify-between"><span>✋ Raise Hand</span><kbd className="px-2 py-1 bg-muted rounded">R</kbd></div>
              <div className="flex justify-between"><span>💬 Toggle Chat</span><kbd className="px-2 py-1 bg-muted rounded">C</kbd></div>
              <div className="flex justify-between"><span>🔴 Start/Stop Recording</span><kbd className="px-2 py-1 bg-muted rounded">E</kbd></div>
              <div className="flex justify-between"><span>🎨 Toggle Services Hub</span><kbd className="px-2 py-1 bg-muted rounded">H</kbd></div>
              <div className="flex justify-between"><span>❌ Leave Meeting</span><kbd className="px-2 py-1 bg-muted rounded">Esc</kbd></div>
            </div>
          </div>
        </div>
      )}
...
const ParticipantTileSkeleton = () => (
  <div className="relative min-h-0 overflow-hidden rounded-2xl bg-card/60 animate-pulse">
...
export default function MeetingRoom({
  ...
  const isOverlayPanelOpen = showChat || showParticipants || showAppsHub;
  ...
  useEffect(() => {
    if (onPanelStateChange) {
      onPanelStateChange(showChat, showParticipants, showAppsHub);
    }
  }, [showChat, showParticipants, showAppsHub, onPanelStateChange]);
  ...
  const toggleCamera = useCallback(async () => {
    ...
    try {
      ...
      refreshParticipants();
      showToast(nextEnabled ? "Camera turned on" : "Camera turned off", "success");
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : "Unable to toggle camera.";
      setMediaError(errorMsg);
      showToast(errorMsg, "error");
    }
  }, [localCameraOn, refreshParticipants, videoDeviceId, showToast]);
  ...
  const toggleMic = useCallback(async () => {
    ...
    try {
      ...
      refreshParticipants();
      showToast(nextEnabled ? "Microphone turned on" : "Microphone turned off", "success");
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : "Unable to toggle microphone.";
      setMediaError(errorMsg);
      showToast(errorMsg, "error");
    }
  }, [audioDeviceId, localMicOn, refreshParticipants, showToast]);
  ...
  const toggleScreenShare = useCallback(async () => {
    ...
    try {
      if (!localScreenOn && otherActiveScreenShareCount >= 2) {
        const errorMsg = "Maximum 2 participants can share screens at once. Ask one person to stop sharing.";
        setMediaError(errorMsg);
        showToast(errorMsg, "error");
        return;
      }
      ...
      refreshParticipants();
      showToast(!localScreenOn ? "Screen sharing started" : "Screen sharing stopped", "success");
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : "Unable to toggle screen sharing.";
      setMediaError(errorMsg);
      showToast(errorMsg, "error");
    }
  }, [localScreenOn, otherActiveScreenShareCount, refreshParticipants, showToast]);
  ...
  const toggleRaiseHand = () => {
    ...
    showToast(value ? "Hand raised" : "Hand lowered", "info");
  };
  ...
  const openWhiteboard = (mode: WhiteboardMode) => {
    ...
    if (mode === "annotate" && sharingParticipants.length === 0) {
      const errorMsg = "Start screen share first, then use Annotate.";
      setMediaError(errorMsg);
      showToast(errorMsg, "error");
      return;
    }
    ...
    showToast(`${mode === "whiteboard" ? "Whiteboard" : "Annotation"} session started`, "success");
  };
  ...
  const closeWhiteboard = () => {
    ...
    showToast("Whiteboard session closed", "info");
  };
  ...
  }, [toggleMic, toggleCamera, toggleRaiseHand, recordingState, onLeave]);
  ...
  const respondQuiz = (quizId: string, optionId: string) => {
    ...
    if (didRespond && localIdentity) {
      ...
      showToast("Quiz response submitted", "success");
    }
  };
  ...
  const uploadUiRecordingToS3 = useCallback(async (blob: Blob, filename: string) => {
    const uploadViaServerRoute = async () => {
      ...
      if (!response.ok) {
        throw new Error(
          `UI recording server-upload failed (${response.status}): ${payload.error || "unknown error"
          }`
        );
      }
      return payload.url || "";
    ...
    if (!putRes.ok) {
      ...
      throw new Error(
        `UI recording S3 PUT failed (${putRes.status}): ${details.slice(0, 220) || putRes.statusText || "unknown error"
        }`
      );
    }
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      setRecordingMessage(errorMsg);
      showToast(errorMsg, "error");
    }
  }, [recordingState, roomName, setLocalRecordingAttribute, startUiRootCapture, stopUiRootCapture, showToast]);
  ...
  }, [recordingEgressId, recordingState, roomName, saveResourceToS3, setLocalRecordingAttribute, stopUiRootCapture, showToast]);

  const buildSharedNotesJson = useCallback(
    () =>
      JSON.stringify(
        {
          room: roomName,
  ...
  const downloadSharedNotes = () => {
    ...
    showToast("Shared notes downloaded", "success");
  };
  ...
  }, [buildSharedNotesJson, roomName, saveResourceToS3]);

  const buildActionItemsCsv = useCallback(() => {
    const rows = [
  ...
  const downloadActionItemsCsv = () => {
    ...
    showToast("Action items downloaded", "success");
  };
  ...
  }, [buildActionItemsCsv, roomName, saveResourceToS3]);
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center bg-background text-foreground">
  ...
  return (
    <div className="relative h-[100dvh] min-h-[100dvh] flex flex-col bg-background text-foreground overflow-hidden">
      {/* Toast Notifications */}
      {toast && (
        <Toast
      ...
      {/* Keyboard Shortcuts Help - Hidden when overlay panel is open */}
      {!isOverlayPanelOpen && <ShortcutsHelp />}

      {/* Live Code Modal */}
      {showLiveCode && (
        <div className={`fixed inset-0 z-${Z_INDEX.MODAL} bg-black/80 p-2 md:p-4`}>
          <div className="h-full w-full bg-gray-900 rounded-xl p-2 md:p-3 flex flex-col">
            <div className="flex justify-between items-center mb-2 flex-shrink-0">
              <span className="text-white font-semibold text-sm md:text-base">Live Coding</span>
              <button
                onClick={() => setShowLiveCode(false)}
                className="text-white bg-red-500 px-2 py-1 md:px-3 md:py-1 rounded text-sm"
              >
                Close
              </button>
            </div>
            <div className="flex-1 min-h-0">
              <VSCodeEditor room={roomRef.current} roomId={roomName} />
            </div>
          </div>
        </div>
      )}
      ...
      {/* Caption Component */}
      {showCaption && (
        <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 rounded-lg border border-border 
          bg-card/90 px-3 py-2 text-xs text-foreground shadow-sm z-${Z_INDEX.CAPTION}
          max-w-[90vw] md:max-w-md text-center whitespace-normal`}>
      ...
      {reactions.map((reaction, i) => (
        <div
          key={`${reaction.id}-${i}`}
          className={`fixed bottom-32 left-1/2 -translate-x-1/2 text-3xl animate-bounce 
            z-${Z_INDEX.REACTION} pointer-events-none`}
      ...
      {showAppsHub && (
        <div
          className={`fixed right-2 top-16 md:right-3 z-${Z_INDEX.MODAL} rounded-2xl border border-border 
            bg-card/95 shadow-2xl backdrop-blur-md transition-all duration-300
            ${minimizedAppsHub ? "w-auto min-w-[56px] p-2" : "w-[calc(100vw-2rem)] md:w-[min(92vw,28rem)] max-h-[90vh] overflow-y-auto p-3"}`}
        >
                              ...
                              {quiz.options.map((option) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`flex w-full items-center justify-between rounded border px-2 py-1 text-left ${isCorrectSelection
                          ...
                          <div key={qa.id} className="flex items-center justify-between rounded-lg border border-border bg-card px-2 py-1.5 text-xs">
                            ...
                            <div className="ml-2 flex items-center gap-1">
                              <button
                                ...
                                className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] ${qa.votesByUser[localIdentity] === "up" ? "bg-emerald-500/20 text-emerald-700" : "bg-accent"
                                  }`}
                              >
                              ...
                              <button
                                ...
                                className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] ${qa.votesByUser[localIdentity] === "down" ? "bg-rose-500/20 text-rose-700" : "bg-accent"
                                  }`}
                              >
      ...
      {/* Connection Status - Hidden when overlay panel is open */}
      {!isOverlayPanelOpen && (
        <div className={`fixed top-3 left-3 z-${Z_INDEX.BASE}`}>
      ...
      {/* Error Message */}
      {mediaError && (
        <div className={`fixed top-20 left-1/2 transform -translate-x-1/2 z-${Z_INDEX.NOTIFICATION}
          rounded-md border border-red-400/60 bg-red-900/70 px-3 py-2 text-xs text-red-50 max-w-[90vw] text-center`}>
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden">
        <div className="relative flex-1 min-h-0 overflow-y-auto p-2 md:p-4 pt-14 pb-24 md:pb-28">
          {whiteboardActive ? (
            <div className="grid h-full grid-cols-1 gap-2 xl:grid-cols-[minmax(0,1fr)_20rem]">
              ...
              <div className="h-32 min-h-[8rem] sm:h-36 sm:min-h-[9rem] xl:h-full xl:min-h-0">
                <div
                  className={`grid h-full gap-2 overflow-x-auto pb-1 grid-flow-col auto-cols-[minmax(7.5rem,10rem)] sm:auto-cols-[minmax(9rem,1fr)] xl:overflow-visible xl:pb-0 xl:grid-flow-row xl:auto-cols-auto ${stripVisibleParticipants.length > 2
            ...
            <div className="grid h-full grid-cols-1 gap-2 xl:grid-cols-[minmax(0,1fr)_20rem]">
              <div
                className={`grid h-full min-h-0 gap-2 ${sharingParticipants.length === 1 ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2"
                  }`}
              >
            ...
            <div
              className={`relative grid h-full gap-2 ${mainGridVisibleParticipants.length === 1
                ? "grid-cols-1"
        ...
        {showParticipants && (
          <>
            <button
              type="button"
              aria-label="Close participants backdrop"
              onClick={() => setShowParticipants(false)}
              className={`fixed inset-0 z-${Z_INDEX.MODAL_BACKDROP} bg-black/40 lg:hidden`}
            />
            <div
              className={`
        fixed inset-y-0 right-0 z-${Z_INDEX.MODAL} w-full max-w-[85vw]
        border-l border-border bg-background/95 backdrop-blur-sm
        sm:max-w-80 transform transition-transform duration-300
        lg:static lg:inset-y-0 lg:inset-x-auto lg:w-80 lg:border-l-0 lg:bg-transparent lg:transform-none
      `}
            >
              <button
                type="button"
                ...
                onClick={() => setShowParticipants(false)}
                className="absolute right-3 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-secondary-foreground hover:bg-secondary/80"
              >
                <X className="h-4 w-4" />
              </button>
      ...
      fixed inset-y-0 right-0 z-${Z_INDEX.MODAL} w-full max-w-[85vw]
      border-l border-border bg-background/95 backdrop-blur-sm
      sm:max-w-[32rem] transform transition-transform duration-300
      lg:static lg:w-[28rem] lg:bg-transparent lg:transform-none
    `}
          >
            <button
              type="button"
      ...
      {/* Bottom Controls */}
      <div className={`fixed bottom-0 left-0 right-0 z-${Z_INDEX.CONTROLS} border-t border-border 
        bg-background/95 backdrop-blur-md safe-area-bottom`}>

        {/* Top action buttons */}
        <div className="absolute -top-12 right-2 flex items-center gap-1.5 md:right-4 md:gap-2 bg-background/80 rounded-full p-1 backdrop-blur-sm">
          <TouchTarget>
          ...
          <TouchTarget>
            <Button
              ...
              className="rounded-full h-10 w-10 p-0 bg-secondary text-secondary-foreground hover:bg-secondary/80"
          ...
          <TouchTarget>
            <Button
              ...
              className="rounded-full h-10 w-10 p-0 bg-secondary text-secondary-foreground hover:bg-secondary/80"
              aria-label={showParticipants ? "Close participants" : "Open participants"}
            >
              {showParticipants ? <X className="h-4 w-4" /> : <Users className="h-4 w-4" />}
            </Button>
          </TouchTarget>
          ...
          <TouchTarget>
            <Button
              ...
              className="relative rounded-full h-10 w-10 p-0 bg-secondary text-secondary-foreground hover:bg-secondary/80"
              aria-label={showChat ? "Close chat" : "Open chat"}
            >
              {showChat ? <X className="h-4 w-4" /> : <MessageSquare className="h-4 w-4" />}
        ...
        <div className="mx-auto flex min-h-[72px] w-full items-center justify-start gap-1.5 
          overflow-x-auto px-2 py-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)]
          md:justify-center md:gap-3 md:px-4 md:py-3 md:overflow-visible
          [&::-webkit-scrollbar]:hidden scrollbar-none">

          <TouchTarget>
            <button
              onClick={() => void toggleMic()}
              title={localMicOn ? "Microphone On (M)" : "Microphone Off (M)"}
              className="flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full bg-secondary text-secondary-foreground transition-colors hover:bg-secondary/80 flex-shrink-0"
            >
          ...
          <TouchTarget>
            <button
              ...
              title={localCameraOn ? "Camera On (V)" : "Camera Off (V)"}
              className="flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full bg-secondary text-secondary-foreground transition-colors hover:bg-secondary/80 flex-shrink-0"
            >
          ...
          <TouchTarget>
            <button
              ...
              className={`flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full transition flex-shrink-0 ${localScreenOn
          ...
          <TouchTarget>
            <button
              ...
              title="Live Code"
              className={`flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full transition flex-shrink-0 ${showLiveCode
          ...
          <TouchTarget>
            <button
              ...
              title="Raise Hand (R)"
              className={`flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full transition flex-shrink-0 ${handRaised
          ...
          <div className="relative">
            <TouchTarget>
              <button
                ...
                title="Reactions"
                className={`flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full transition flex-shrink-0 ${showReactions
                  ? "bg-primary text-primary-foreground hover:bg-primary/80"
                  : "bg-secondary text-secondary-foreground hover:bg-secondary/80"
                  }`}
              >
            ...
            {showReactions && (
              <div className={`absolute bottom-12 left-1/2 z-${Z_INDEX.DROPDOWN} flex -translate-x-1/2 
                gap-2 rounded-full border border-border bg-card/95 px-3 py-2 shadow-lg whitespace-nowrap`}>
                {["❤️", "😀", "😂", "😢", "👍"].map((e) => (
                  <button
                    key={e}
                    onClick={() => {
          ...
          <TouchTarget>
            <button
              ...
              title="Live Caption"
              className={`flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full transition flex-shrink-0 ${showCaption
                ? "bg-primary text-primary-foreground hover:bg-primary/80"
                : "bg-secondary text-secondary-foreground hover:bg-secondary/80"
                }`}
            >
          ...
          <TouchTarget>
            <button
              ...
              className={`flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full transition flex-shrink-0 
                disabled:opacity-60 disabled:cursor-not-allowed ${recordingState === "recording"
          ...
          <TouchTarget>
            <button
              ...
              className="flex h-10 w-10 min-w-[40px] items-center justify-center rounded-full bg-red-600 text-white transition hover:bg-red-700 flex-shrink-0"
            >
      ...
      {activeRecordingNames.length > 0 && (
        <div className={`fixed left-1/2 top-16 transform -translate-x-1/2 z-${Z_INDEX.RECORDING_INDICATOR}
          rounded-full bg-black/70 px-3 py-1 text-xs text-white whitespace-nowrap`}>
      ...
      {/* Recording Message */}
      {recordingMessage && (
        <div className={`fixed bottom-20 left-1/2 transform -translate-x-1/2 z-${Z_INDEX.NOTIFICATION}
          rounded-md bg-black/65 px-3 py-1 text-xs text-white max-w-[90vw] text-center
          md:bottom-24 md:max-w-md`}>
          {recordingMessage}
        </div>
      )}
    </div>
